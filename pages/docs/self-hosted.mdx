...
- **`deviceplane/deviceplane:<version>-with-db`** Contains both the controller and an embedded MySQL with the schema preloaded. Used for the two documented self-hosted use cases. 
- **`deviceplane/deviceplane:<version>`** Contains only the controller. Used when you want to connect to an external MySQL database. 
