...
This quick start guide will teach you how to provision a device and then deploy to it. Specifically this guide will cover the following.

1. Creating a project
2. Provisioning a device
3. Creating an application
4. Creating an initial release

The only prerequisite is having access to a device ready to be managed by Device Plane and running a recent version of Docker.
...
From the homepage of the Device Plane app click on "Create Project". Enter a project name and then click on submit.

## Provisioning a device

Now that we have an empty project, our next step is to add a device. From the "Devices" page click on "Add Device". A unique Docker command will be generated. Running this command on your device will download our agent image and then start the agent as a new container.
