...
```
docker run -d --restart=unless-stopped -p 8080:8080 deviceplane/deviceplane:1.11.0-with-db
```
...
```shell
curl https://install.deviceplane.com | VERSION=<version> PROJECT=<project> REGISTRATION_TOKEN=<registration_token> CONTROLLER=http://<backend_ip_address>:8080/api bash
```
