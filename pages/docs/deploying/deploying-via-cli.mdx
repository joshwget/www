...
In addition to deploying via the UI, Deviceplane includes deployment features in its [CLI](./cli). These docs describe how to use the CLI to deploy manually, but the same commands can be used to setup [deployment pipelines](./deploying/pipelines).
...
$ deviceplane deploy --project <PROJECT_ID_OR_NAME>
