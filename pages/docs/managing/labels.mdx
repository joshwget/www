...
Device labels are the main unit of organization for devices. Labels are simply user-defined key value pairs that are assignable to devices individually, or in bulk, as part of [a provisioning workflow](/docs/provisioning/bulk-provisioning).
...
By themselves, labels are informational, and only moderately useful. However, labels are heavily incorporated into various Deviceplane features. Now, using your labels, you can [query devices](/docs/managing/filters), from the UI and CLI, as well as [conditionally scheduling applications](/docs/deploying/scheduling) and [pinning application releases](/docs/deploying/release-pinning).
